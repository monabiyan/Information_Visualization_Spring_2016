...
######### All UNIQUE Changing Type to all American
setwd("/Users/mohsennabian/Desktop")
ALL_UNIQUE<-read.csv(file="ALL_UNIQUE.csv",stringsAsFactors=FALSE);
sum(ALL_UNIQUE$NAME=="American(New)")

for (i in 1:nrow(ALL_UNIQUE))
{
  if (ALL_UNIQUE$TYPE[i]=="American (New)")
  {ALL_UNIQUE$TYPE[i]="American"}
  if (ALL_UNIQUE$TYPE[i]=="American (Traditional)")
  {ALL_UNIQUE$TYPE[i]="American"}
}
...
################Nationality Map
ALL_UNIQUE_some<-ALL_UNIQUE[(ALL_UNIQUE$TYPE=="Polish")|(ALL_UNIQUE$TYPE=="Turkish"|ALL_UNIQUE$TYPE=="Persian/Iranian"|ALL_UNIQUE$TYPE=="Kosher"|ALL_UNIQUE$TYPE=="Latin American"|ALL_UNIQUE$TYPE=="Portuguese"),]
  
myMap1 <-get_map(location="Boston",zoom=11,color='bw',source="google",maptype="roadmap",crop=FALSE)

g1<-ggmap(myMap1)+geom_point(aes(x = longitude ,y = latitude, color=ALL_UNIQUE_some$TYPE), data =ALL_UNIQUE_some,size=6)
g1<-g1+xlab("Longitude")+ylab("Latitude")+ggtitle("Restaurants by Countries/Type")
#g1<-g1+scale_colour_continuous(low = "red", high = "blue", space = "Lab", guide = "colorbar")
g1
#############################
zip_info<-read.csv(file="zip_info.csv",stringsAsFactors=FALSE);
require(visreg)
plot(zip_info$mean_star ~ zip_info$mean_price)
fit <- lm(zip_info$mean_star ~ zip_info$mean_price)
summary(fit)



ggplot(zip_info, aes(x=mean_star, y=mean_price)) +geom_point(shape=1) +geom_smooth(method=lm)
summary(lm(zip_info$mean_star ~ zip_info$mean_price))


ggplot(zip_info, aes(x=median_salary,y=mean_price)) +geom_point(shape=19) +geom_smooth(method=lm)
summary(lm(zip_info$mean_price ~ zip_info$median_star))


ggplot(ALL_UNIQUE, aes(x=ALL_UNIQUE$PRICE,y=ALL_UNIQUE$STAR)) +geom_point(shape=19) +geom_smooth(method=lm)
summary(lm(ALL_UNIQUE$PRICE ~ ALL_UNIQUE$STAR))

ggplot(ALL_UNIQUE, aes(x=ALL_UNIQUE$REVIEW_COUNT,y=ALL_UNIQUE$STAR)) +geom_point(shape=19) +geom_smooth(method=lm)
summary(lm(ALL_UNIQUE$REVIEW_COUNT ~ ALL_UNIQUE$STAR))
nrow(zip_info)
